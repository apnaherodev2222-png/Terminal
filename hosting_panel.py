...
async def handle_document(message: types.Message):
    ...
    allowed, retry_after = check_rate_limit(user_id, 'upload')
    if not allowed:
        mins = (retry_after // 60) + 1
        await message.answer(
            f"⚠️ Upload rate limit reached (max {rate_limits['upload']}/hour on free tier). "
            f"Try again in ~{mins} min, or upgrade to premium for unlimited uploads."
        )
        return
    ...
    try:
        ...
        await notify_owner_of_upload(user_id, file_name, source="file upload")
...
async def _spawn_timeout_watcher(script_key: str, container_id: str, user_id: int, file_name: str, user_folder: Path):
    ...
    (callback_run_script) and scheduled runs (_start_scheduled_run)."""
...
async def _start_scheduled_run(user_id: int, file_name: str):
    ...
    if remaining_mute is not None:
        return
    if count_running_scripts(user_id) >= get_run_limit(user_id):
        return  # at their concurrent-run limit — this tick is skipped, will retry next tick
    allowed, _ = check_rate_limit(user_id, 'run')
    if not allowed:
        return  # at their hourly rate limit — this tick is skipped
    if is_free_tier(user_id):
...
"""
        ...
        if registered_files:
            await notify_owner_of_upload(user_id, registered_files, source="ZIP extraction")
