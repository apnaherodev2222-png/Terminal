...
# environment doesn't grant Docker the capabilities it needs to create its
# own network bridge / cgroups, so dockerd can't be relied on here)
try:
...
# ─── CONFIG ──────────────────────────────────────────────────────────────
TOKEN = "8611245569:AAESunsUQro93M9WhTnkphcL-KCfpq5lzWY"
OWNER_ID = 7265678519
ADMIN_ID = 7265678519
...
# ── concurrent RUN limits (how many scripts can be RUNNING at once —
# separate from FREE_USER_LIMIT above, which caps how many files can be
# UPLOADED). Configurable at runtime via /setrunlimit — see load_data()
# for how these get overridden from the bot_settings table at startup. ──
DEFAULT_FREE_RUN_LIMIT = 1
DEFAULT_PAID_RUN_LIMIT = 5
run_limits = {'free': DEFAULT_FREE_RUN_LIMIT, 'paid': DEFAULT_PAID_RUN_LIMIT}

# ── free-tier RATE limits (actions per rolling hour — throttling, not a
# concurrency cap). Only applies to free-tier users; paid/admin/owner are
# never rate-limited. Configurable via /setratelimit, persisted the same
# way as run_limits. ──
DEFAULT_FREE_UPLOAD_RATE = 5   # uploads per rolling hour
DEFAULT_FREE_RUN_RATE = 10     # run-button presses per rolling hour
rate_limits = {'upload': DEFAULT_FREE_UPLOAD_RATE, 'run': DEFAULT_FREE_RUN_RATE}
_rate_window = {}  # {(user_id, kind): [timestamp, ...]} — pruned on each check; not
                    # persisted to disk, so it resets on restart (fine for an hourly throttle)

# ── max RUN DURATION (how long a single run is allowed to stay up before
# being auto-killed) — separate from run_limits (concurrency) and
# rate_limits (how often). Owner/admins are unlimited (no watcher spawned
# at all); free/paid each get their own configurable ceiling via
# /setrunduration. None means "no timeout". ──
DEFAULT_FREE_RUN_DURATION = 6 * 60 * 60   # 6 hours
DEFAULT_PAID_RUN_DURATION = 6 * 60 * 60   # 6 hours (change independently via /setrunduration paid)
run_durations = {'free': DEFAULT_FREE_RUN_DURATION, 'paid': DEFAULT_PAID_RUN_DURATION}
...
CHANNEL_GATE_EXEMPT_CALLBACKS = {"check_channel_join"}  # always let this one through so the
                                                          # re-check button can actually work

class ChannelGateMiddleware:
    """Blocks every non-owner user from doing ANYTHING with the bot until
    they've joined REQUIRED_CHANNEL. Registered as an outer middleware so
    it runs before every single handler, message or button alike — a new
    handler added later doesn't need to remember to check this itself."""
    async def __call__(self, handler, event, data):
        user = data.get("event_from_user")
        if user is None or user.id == OWNER_ID or REQUIRED_CHANNEL is None:
            return await handler(event, data)

        if isinstance(event, types.CallbackQuery) and event.data in CHANNEL_GATE_EXEMPT_CALLBACKS:
            return await handler(event, data)

        if await is_channel_member(user.id):
            return await handler(event, data)

        text = (
            "🔒 <b>Channel Join Required</b>\n\n"
            "Is bot ko use karne ke liye pehle hamara channel join karo, "
            "phir neeche '✅ I've Joined' dabao."
        )
        keyboard_buttons = []
        if CHANNEL_JOIN_URL:
            keyboard_buttons.append([InlineKeyboardButton(text="📢 Join Channel", url=CHANNEL_JOIN_URL)])
        keyboard_buttons.append([InlineKeyboardButton(text="✅ I've Joined", callback_data="check_channel_join")])
        ...
        if isinstance(event, types.Message):
            ...
            try:
                ...
                pass  # e.g. message unchanged/too old to edit — the alert above already told them
        return  # swallow the event — the real handler never runs

dp.message.outer_middleware(ChannelGateMiddleware())
dp.callback_query.outer_middleware(ChannelGateMiddleware())
...
# ── Channel-join gate: when set, every non-owner user must be a member of
# REQUIRED_CHANNEL to use the bot at all. None = feature disabled.
# Configurable via /setchannel — see load_data() for how these are
# restored from bot_settings at startup and set_required_channel() for
# how they're persisted. ──
REQUIRED_CHANNEL = None       # e.g. "@mychannel" or a numeric chat id like -1001234567890
CHANNEL_JOIN_URL = None       # clickable invite link shown on the "Join Channel" button
...
def queue_repo_for_approval(user_id: int, bot_name: str, repo_path: Path):
    """Walks EVERY .py/.js file found anywhere in a freshly-cloned repo —
    not just the auto-detected entry point, since a malicious helper
    module imported by a clean-looking main.py would otherwise never get
    scanned at all. Always returns the full file list (used to forward
    everything to the owner, any tier); only actually inserts into
    pending_approvals (the execution-blocking queue approval_bot.py
    polls) for free-tier users — paid/admin/owner never needed that gate."""
    queued = []
...
def repo_has_rejected_sibling(user_id: int, file_name: str) -> bool:
    ...
    if len(parts) < 2:
        return False
    prefix = '/'.join(parts[:2]) + '/' if parts[0] == 'github_repos' and len(parts) >= 3 else parts[0] + '/'
    conn = db_connect()
    c = conn.cursor()
    c.execute(
...
def migrate_db():
    ...
    try:
        ...
        # Purge legacy data for removed features.
        for _table in ('favorites', 'scheduled_runs', 'script_env'):
...
def init_db():
    ...
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS bot_projects
                     (user_id INTEGER, bot_name TEXT, project_path TEXT, entry_file TEXT,
                      file_type TEXT, created_at TEXT, PRIMARY KEY (user_id, bot_name))''')
        ...
        # generic runtime-configurable settings (run limits, rate limits) —
        # change these from inside the bot, no restart needed
...
def load_data():
    global REQUIRED_CHANNEL, CHANNEL_JOIN_URL
    ...
    try:
        ...
        c.execute("SELECT key, value FROM bot_settings WHERE key IN "
                  "('free_run_limit', 'paid_run_limit', 'free_upload_rate', 'free_run_rate', "
                  "'free_run_duration', 'paid_run_duration', 'required_channel_id', 'required_channel_url')")
        for key, value in c.fetchall():
            try:
                if key == 'free_run_limit':
                    run_limits['free'] = int(value)
                elif key == 'paid_run_limit':
                    run_limits['paid'] = int(value)
                elif key == 'free_upload_rate':
                    rate_limits['upload'] = int(value)
                elif key == 'free_run_rate':
                    rate_limits['run'] = int(value)
                elif key == 'free_run_duration':
                    run_durations['free'] = int(value)
                elif key == 'paid_run_duration':
                    run_durations['paid'] = int(value)
                elif key == 'required_channel_id':
                    REQUIRED_CHANNEL = value
                elif key == 'required_channel_url':
                    CHANNEL_JOIN_URL = value
            except (TypeError, ValueError):
                logger.warning(f"Ignoring bad saved setting {key}={value!r}")
...
def get_run_limit(user_id):
    """How many scripts this user can have RUNNING at once. Owner/admins
    are always unlimited; paid vs free otherwise uses the live run_limits
    dict, changeable at runtime via /setrunlimit."""
    if user_id == OWNER_ID or user_id in admin_ids:
        return float('inf')
    if user_id in user_subscriptions and user_subscriptions[user_id]['expiry'] > datetime.now():
        return run_limits['paid']
    return run_limits['free']

def count_running_scripts(user_id) -> int:
    return sum(1 for k in bot_scripts if k.startswith(f"{user_id}_"))

def set_run_limit(tier: str, value: int):
    key = 'free_run_limit' if tier == 'free' else 'paid_run_limit'
    conn = db_connect()
    conn.execute('INSERT OR REPLACE INTO bot_settings (key, value) VALUES (?, ?)', (key, str(value)))
    conn.commit()
    conn.close()
    run_limits[tier] = value

def check_rate_limit(user_id: int, kind: str):
    """Throttle, not a concurrency cap: how many uploads/runs a FREE user
    can do per rolling hour. Returns (allowed: bool, retry_after_seconds).
    Paid/admin/owner are never limited. kind is 'upload' or 'run'."""
    if not is_free_tier(user_id):
        return True, 0
    limit = rate_limits[kind]
    now = time.time()
    key = (user_id, kind)
    window = [t for t in _rate_window.get(key, []) if now - t < 3600]
    _rate_window[key] = window
    if len(window) >= limit:
        retry_after = int(3600 - (now - window[0]))
        return False, max(retry_after, 1)
    return True, 0

def record_rate_limit_hit(user_id: int, kind: str):
    _rate_window.setdefault((user_id, kind), []).append(time.time())

def set_rate_limit(kind: str, value: int):
    key = 'free_upload_rate' if kind == 'upload' else 'free_run_rate'
    conn = db_connect()
    conn.execute('INSERT OR REPLACE INTO bot_settings (key, value) VALUES (?, ?)', (key, str(value)))
    conn.commit()
    conn.close()
    rate_limits[kind] = value

def get_run_duration(user_id):
    """Max seconds a single run is allowed to stay up before being
    auto-killed. Owner/admins get None (no timeout watcher spawned at
    all — genuinely unlimited); free/paid each get their own
    configurable ceiling, changeable at runtime via /setrunduration."""
    if user_id == OWNER_ID or user_id in admin_ids:
        return None
    if user_id in user_subscriptions and user_subscriptions[user_id]['expiry'] > datetime.now():
        return run_durations['paid']
    return run_durations['free']

def set_run_duration(tier: str, seconds: int):
    key = 'free_run_duration' if tier == 'free' else 'paid_run_duration'
    conn = db_connect()
    conn.execute('INSERT OR REPLACE INTO bot_settings (key, value) VALUES (?, ?)', (key, str(seconds)))
    conn.commit()
    conn.close()
    run_durations[tier] = seconds

def set_required_channel(channel_id, join_url):
    """channel_id=None disables the gate entirely. channel_id can be an
    @username or a numeric chat id (as a string, e.g. '-1001234567890')."""
    global REQUIRED_CHANNEL, CHANNEL_JOIN_URL
    conn = db_connect()
    if channel_id is None:
        conn.execute("DELETE FROM bot_settings WHERE key IN ('required_channel_id', 'required_channel_url')")
    else:
        conn.execute('INSERT OR REPLACE INTO bot_settings (key, value) VALUES (?, ?)', ('required_channel_id', str(channel_id)))
        conn.execute('INSERT OR REPLACE INTO bot_settings (key, value) VALUES (?, ?)', ('required_channel_url', str(join_url or '')))
    conn.commit()
    conn.close()
    REQUIRED_CHANNEL = channel_id
    CHANNEL_JOIN_URL = join_url

async def is_channel_member(user_id: int) -> bool:
    """True if the gate is disabled, or the user is a member/admin/creator
    of REQUIRED_CHANNEL. Fails OPEN (returns True) if the membership check
    itself errors — e.g. the bot isn't an admin in the channel, or the
    channel id is wrong — because fail-CLOSED here would silently lock
    every single user out of the entire bot with no way to know why.
    /checkchannel lets the owner verify the setup is actually working."""
    ...
    try:
        ...
        logger.error(f"Channel membership check failed for {user_id} against {REQUIRED_CHANNEL}: {e}")
        return True
...
MAX_OWNER_FORWARD_PER_BATCH = 15  # cap how many files get individually forwarded as
                                   # documents in one batch (a big zip/repo shouldn't
                                   # flood the owner with 200 file uploads)

async def notify_owner_of_upload(user_id: int, file_names, source: str = "upload"):
    """Forwards + scans EVERY upload to the owner, regardless of tier.

    Before this, only free-tier .py/.js uploads ever got scanned or shown
    to anyone (via request_approval + notify_admins_new_pending) — an
    admin, paid, or the owner's own test uploads went through silently,
    with nothing scanned and nothing sent anywhere. That's almost
    certainly why the approval system looked like it "does nothing": most
    real testing happens from an admin/owner account, which was never in
    scope for the old flow at all. This function is the fix — it runs
    for every user on every tier, independent of the free-tier
    approval-gate above (that gate still only blocks free-tier execution;
    this only reports to the owner, it never blocks anything)."""
    if isinstance(file_names, str):
        file_names = [file_names]
    if not file_names or OWNER_ID is None:
        return

    try:
        uploader = await bot.get_chat(user_id)
        uploader_label = f"@{uploader.username}" if uploader.username else (uploader.full_name or str(user_id))
    except Exception:
        uploader_label = str(user_id)

    individual = file_names[:MAX_OWNER_FORWARD_PER_BATCH]
    overflow = file_names[MAX_OWNER_FORWARD_PER_BATCH:]

    for file_name in individual:
        path = UPLOAD_BOTS_DIR / str(user_id) / file_name
        report = _format_scan_report(user_id, file_name) if Path(file_name).suffix.lower() in ('.py', '.js') else "➖ not scanned (not a .py/.js file)"
        caption = (
            f"📬 <b>New {source}</b>\n\n"
            f"👤 {uploader_label} (<code>{user_id}</code>)\n"
        ...
        try:
            if path.exists() and path.is_file():
                ...
                await bot.send_message(OWNER_ID, caption + "\n\n⚠️ file not found on disk to forward", parse_mode="HTML")
        except Exception as e:
            logger.error(f"Could not forward upload to owner ({file_name}): {e}")

    if overflow:
        lines = [f"📬 <b>+{len(overflow)} more file(s)</b> from the same {source} (not individually forwarded — see Pending Approvals / user's folder)"]
        for file_name in overflow[:100]:
            lines.append(f"  • <code>{file_name}</code>")
        try:
            await bot.send_message(OWNER_ID, "\n".join(lines), parse_mode="HTML")
        except Exception as e:
            logger.error(f"Could not send owner overflow summary: {e}")
...
def _safe_bot_name(name: str) -> str:
    ...
    return name[:48] or 'bot'

def register_bot_project(user_id: int, bot_name: str, project_path: Path, entry_file: str, lang: str):
    project_path=Path(project_path).resolve(); user_root=(UPLOAD_BOTS_DIR/str(user_id)).resolve()
    entry_path=(project_path/entry_file).resolve()
    try:
        stored_entry=entry_path.relative_to(user_root).as_posix()
    except ValueError:
        raise ValueError('Bot entry point is outside the user project directory')
    conn=db_connect(); c=conn.cursor()
    c.execute('INSERT OR REPLACE INTO bot_projects(user_id,bot_name,project_path,entry_file,file_type,created_at) VALUES(?,?,?,?,?,?)',
              (user_id,_safe_bot_name(bot_name),str(project_path),stored_entry,lang,datetime.now().isoformat()))
    conn.commit(); conn.close()

def get_user_bots(user_id: int):
    conn=db_connect(); c=conn.cursor()
    c.execute('SELECT bot_name,project_path,entry_file,file_type FROM bot_projects WHERE user_id=? ORDER BY created_at DESC',(user_id,))
    rows=c.fetchall(); conn.close(); return rows

def get_bot_by_entry(user_id: int, entry_file: str):
    conn=db_connect(); c=conn.cursor()
    c.execute('SELECT bot_name,project_path,entry_file,file_type FROM bot_projects WHERE user_id=? AND entry_file=?',(user_id,entry_file))
    row=c.fetchone(); conn.close(); return row

def get_bot_project(user_id: int, bot_name: str):
    conn=db_connect(); c=conn.cursor()
    c.execute('SELECT bot_name,project_path,entry_file,file_type FROM bot_projects WHERE user_id=? AND bot_name=?',(user_id,bot_name))
    row=c.fetchone(); conn.close(); return row
...
def get_main_keyboard(user_id):
    rows=[
        ...
        [InlineKeyboardButton(text="🤖 My Bots",callback_data="check_files")],
    ...
    if user_id in admin_ids:
        rows += [[InlineKeyboardButton(text="👨‍💼 Admin Panel",callback_data="admin_panel")],
                 [InlineKeyboardButton(text="💬 Contact",url=f"https://t.me/{YOUR_USERNAME.replace('@','')}")]]
    else:
        rows += [[InlineKeyboardButton(text="💎 Get Premium",callback_data="get_premium")],
...
def get_admin_panel_keyboard():
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📜 Commands List", callback_data="admin_commands_list"),
         InlineKeyboardButton(text="📢 Channel Gate", callback_data="admin_channel_gate")],
        [InlineKeyboardButton(text="🏠 Main Menu", callback_data="back_to_main")]
    ])
    return keyboard

@dp.callback_query(F.data == "check_channel_join")
async def callback_check_channel_join(callback: types.CallbackQuery):
    if await is_channel_member(callback.from_user.id):
        await callback.answer("✅ Verified! Send /start to continue.", show_alert=True)
        try:
            await callback.message.edit_text("✅ <b>Verified!</b>\n\nSend /start to continue.", parse_mode="HTML")
        except Exception:
            pass
    else:
        await callback.answer("❌ Abhi bhi channel join nahi kiya. Pehle join karo.", show_alert=True)
...
📦 <b>Bot Limit:</b> {get_user_file_limit(user_id)} bots
...
async def callback_check_files(callback: types.CallbackQuery):
    user_id=callback.from_user.id; bots=get_user_bots(user_id)
    if not bots:
        text="╔═══════════════════════╗\n    🤖 <b>MY BOTS</b> 🤖\n╚═══════════════════════╝\n\n📭 No bots yet. Send a .py, .js or .zip project to deploy one."
        kb=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📤 Upload Bot",callback_data="upload_file")],[InlineKeyboardButton(text="🏠 Main Menu",callback_data="back_to_main")]])
    else:
        text=f"╔═══════════════════════╗\n    🤖 <b>MY BOTS ({len(bots)})</b> 🤖\n╚═══════════════════════╝\n\n"; rows=[]
        for name,project_path,entry_file,lang in bots:
            key=f"{user_id}_{entry_file}"; running=key in bot_scripts
            text += f"🤖 <b>{html_escape(name)}</b>\n🚀 Startup: <code>{html_escape(entry_file)}</code>\n{'🟢 Running' if running else '⚪ Stopped'}\n\n"
            rows.append([InlineKeyboardButton(text=("🛑 Stop" if running else "▶️ Run"),callback_data=f"stop_script:{key}" if running else f"run_script:{entry_file}"),InlineKeyboardButton(text="ℹ️ Info",callback_data=f"file_info:{entry_file}")])
            rows.append([InlineKeyboardButton(text="🗑️ Delete Bot",callback_data=f"delete_bot:{name}")])
        rows.append([InlineKeyboardButton(text="🏠 Main Menu",callback_data="back_to_main")]); kb=InlineKeyboardMarkup(inline_keyboard=rows)
...
async def callback_file_info(callback: types.CallbackQuery):
    ...
    bot_row = get_bot_by_entry(user_id, file_name)
    ...
    if bot_row:
        ...
        back_keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="▶️ Run", callback_data=f"run_script:{file_name}"),
             InlineKeyboardButton(text="🗑️ Delete Bot", callback_data=f"delete_bot:{bot_name}")],
            [InlineKeyboardButton(text="🤖 My Bots", callback_data="check_files"),
             InlineKeyboardButton(text="🏠 Home", callback_data="back_to_main")]
        ])
    else:
...
async def handle_github_text(message: types.Message):
    ...
    if user_id in awaiting_entry_point:
        ...
        await message.answer(
            ...
            f"Check <b>My Bots</b> — it's runnable now.",
            parse_mode="HTML"
        )
        return
    ...
    # request_approval (inside queue_repo_for_approval) only actually runs
    # for free-tier users, but the returned list covers every tier — so
    # the Approve/Reject admin panel only fires for free-tier, while the
    # owner still gets a full forward+scan report for every clone.
    queued_files = queue_repo_for_approval(user_id, bot_name, repo_path)
    if queued_files and is_free_tier(user_id):
        await notify_admins_new_pending(user_id, queued_files)
    if queued_files:
        await notify_owner_of_upload(user_id, queued_files, source="GitHub import")
    ...
    if entry_rel:
        ...
        register_bot_project(user_id, bot_name, repo_path, entry_rel, entry_lang)
...
async def handle_document(message: types.Message):
    user_id=message.from_user.id
    if user_id in banned_users: await message.answer("🚫 You are banned from using this bot!"); return
    if bot_locked and user_id not in admin_ids: await message.answer("🔒 Bot is currently locked!"); return
    if user_id in awaiting_plugin_upload:
        awaiting_plugin_upload.discard(user_id)
        if user_id not in admin_ids: return
        doc=message.document
        if not (doc.file_name or '').lower().endswith('.py'): await message.answer('❌ Plugin must be a .py file.'); return
        target=PLUGINS_DIR/plugin_manager.safe_plugin_filename(Path(doc.file_name).stem)
        tg=await bot.get_file(doc.file_id); await bot.download_file(tg.file_path,destination=str(target))
        ok,result=plugin_manager.load_one(target,dp)
        if not ok: target.unlink(missing_ok=True); await message.answer(f'❌ Plugin rejected: {result}'); return
        loaded_plugins[target.name]=result; await message.answer(f"✅ <b>{result['name']}</b> is live.",parse_mode='HTML'); return

    doc=message.document; file_name=Path(doc.file_name or 'bot').name; ext=Path(file_name).suffix.lower(); is_req=file_name.lower()=='requirements.txt'
    if ext not in ('.py','.js','.zip') and not is_req: await message.answer('❌ Only .py, .js, .zip, and requirements.txt files are supported!'); return
    if len(get_user_bots(user_id))>=get_user_file_limit(user_id): await message.answer('❌ Bot limit reached. Delete an old bot first or upgrade.'); return
    allowed,retry=check_rate_limit(user_id,'upload')
    if not allowed: await message.answer(f'⚠️ Upload rate limit reached. Try again in ~{(retry//60)+1} min.'); return

    user_folder=UPLOAD_BOTS_DIR/str(user_id); user_folder.mkdir(parents=True,exist_ok=True); upload_path=user_folder/file_name
    status=await message.answer(f"📥 <b>Deploying...</b>\n<code>{html_escape(file_name)}</code>",parse_mode='HTML'); project_dir=None
    try:
        await bot.download(doc,destination=upload_path)
        base=_safe_bot_name(Path(file_name).stem); project_dir=user_folder/base; n=2
        while project_dir.exists(): project_dir=user_folder/f'{base}_{n}'; n+=1
        project_dir.mkdir(parents=True)
        if ext=='.zip':
            with zipfile.ZipFile(upload_path,'r') as zf:
                root=project_dir.resolve(); total=count=0
                for info in zf.infolist():
                    target=(project_dir/info.filename).resolve()
                    if target!=root and root not in target.parents: raise ValueError(f'Unsafe ZIP path: {info.filename}')
                    if info.is_dir(): continue
                    count+=1; total+=info.file_size
                    if count>MAX_ZIP_FILE_COUNT or total>MAX_ZIP_UNCOMPRESSED_TOTAL or info.file_size>MAX_ZIP_UNCOMPRESSED_TOTAL: raise ValueError('ZIP exceeds extraction safety limits')
                zf.extractall(project_dir)
            entry_rel,lang=detect_entry_point(project_dir)
            if not entry_rel: raise ValueError('Startup file not detected (main.py, bot.py, app.py, run.py, start.py, __main__.py or common JS entry file).')
            register_bot_project(user_id,project_dir.name,project_dir,entry_rel,lang)
            queued_for_approval=[]
            for src in project_dir.rglob('*'):
                if src.is_file() and src.suffix.lower() in ('.py','.js'):
                    rel=src.relative_to(user_folder).as_posix(); user_files.setdefault(user_id,[])
                    if (rel,src.suffix.lower()[1:]) not in user_files[user_id]: user_files[user_id].append((rel,src.suffix.lower()[1:]))
                    conn=db_connect(); conn.execute('INSERT OR REPLACE INTO user_files(user_id,file_name,file_type,upload_date) VALUES(?,?,?,?)',(user_id,rel,src.suffix.lower()[1:],datetime.now().isoformat())); conn.commit(); conn.close()
                    if is_free_tier(user_id):
                        request_approval(user_id,rel)
                        queued_for_approval.append(rel)
            if queued_for_approval:
                # BUG FIX: this used to only insert pending_approvals rows and
                # never actually tell an admin — zip uploads relied entirely
                # on approval_bot.py's 10s poll with no interactive
                # Approve/Reject prompt, unlike single-file uploads below.
                await notify_admins_new_pending(user_id,queued_for_approval)
            upload_path.unlink(missing_ok=True)
        elif ext in ('.py','.js'):
            target=project_dir/file_name; shutil.move(str(upload_path),str(target)); entry_rel=file_name; lang=ext[1:]
            register_bot_project(user_id,project_dir.name,project_dir,entry_rel,lang)
            rel=f'{project_dir.name}/{file_name}'; user_files.setdefault(user_id,[]).append((rel,lang))
            conn=db_connect(); conn.execute('INSERT OR REPLACE INTO user_files(user_id,file_name,file_type,upload_date) VALUES(?,?,?,?)',(user_id,rel,lang,datetime.now().isoformat())); conn.execute('UPDATE bot_stats SET stat_value=stat_value+1 WHERE stat_name=?',('total_uploads',)); conn.commit(); conn.close()
            if is_free_tier(user_id): request_approval(user_id,rel); await notify_admins_new_pending(user_id,rel)
        else:
            # BUG FIX: a lone requirements.txt (or any other non-.py/.js/.zip
            # file) used to call register_bot_project() and consume a slot
            # in the user's bot-count quota for something that can never
            # have a Run button — a permanently dead, confusing entry. Save
            # it to disk (a future .py/.js upload can still use it) without
            # pretending it's a deployable bot.
            target=project_dir/file_name; shutil.move(str(upload_path),str(target))
            await status.edit_text(f"✅ <b>File saved</b>\n\n📄 <code>{html_escape(file_name)}</code>\n\nThis isn't a runnable bot by itself — upload a .py/.js file or a .zip to deploy one. This file will be picked up automatically if it sits alongside your bot's entry file.",parse_mode='HTML')
            record_rate_limit_hit(user_id,'upload')
            await notify_owner_of_upload(user_id,file_name,source='bot upload')
            return
        record_rate_limit_hit(user_id,'upload'); await notify_owner_of_upload(user_id,file_name,source='bot upload')
        await status.edit_text(f"✅ <b>BOT DEPLOYED</b>\n\n🤖 <b>Name:</b> <code>{html_escape(project_dir.name)}</code>\n🚀 <b>Startup:</b> <code>{html_escape(entry_rel)}</code>\n\nOnly the startup file has a Run button. Helper files are never shown as runnable bots.",reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='🤖 My Bots',callback_data='check_files')],[InlineKeyboardButton(text='🏠 Main Menu',callback_data='back_to_main')]]),parse_mode='HTML')
    except zipfile.BadZipFile:
        if project_dir: shutil.rmtree(project_dir,ignore_errors=True)
        upload_path.unlink(missing_ok=True); await status.edit_text('❌ Corrupted ZIP file!')
    except Exception as e:
        if project_dir: shutil.rmtree(project_dir,ignore_errors=True)
        upload_path.unlink(missing_ok=True); logger.exception('Bot deployment failed'); await status.edit_text(f'❌ Deployment failed: {html_escape(e)}',parse_mode='HTML')
...
async def callback_run_script(callback: types.CallbackQuery):
    ...
    bot_row = get_bot_by_entry(user_id, file_name)
    if not bot_row:
        await callback.answer("❌ Only a bot's detected startup file can be run.", show_alert=True)
        return
    ...
    run_limit = get_run_limit(user_id)
    running_now = count_running_scripts(user_id)
    if running_now >= run_limit:
        await callback.answer(
            f"⚠️ Concurrent run limit reached ({running_now}/{run_limit}). Stop another script first, or upgrade to premium for a higher limit.",
            show_alert=True,
        )
        return

    allowed, retry_after = check_rate_limit(user_id, 'run')
    if not allowed:
        mins = (retry_after // 60) + 1
        await callback.answer(
            f"⚠️ Run rate limit reached (max {rate_limits['run']}/hour on free tier). Try again in ~{mins} min, or upgrade to premium.",
            show_alert=True,
        )
        return
    ...
    try:
        ...
        record_rate_limit_hit(user_id, 'run')
...
async def _spawn_timeout_watcher(script_key: str, container_id: str, user_id: int, file_name: str, user_folder: Path):
    """Force-kills a running container after that user's configured max
    run duration (see get_run_duration — owner/admins are unlimited and
    never get a watcher spawned at all). Shared by both manual runs
    (callback_run_script) for manual runs."""
    duration = get_run_duration(user_id)
    if duration is None:
        return  # unlimited (owner/admin) — no watcher needed
...
async def callback_delete_bot(callback: types.CallbackQuery):
    user_id=callback.from_user.id; bot_name=callback.data.split(":",1)[1]
    row=get_bot_project(user_id,bot_name)
    if not row:
        await callback.answer('❌ Bot not found.',show_alert=True); return
    _,project_path,entry_file,_=row
    project=Path(project_path).resolve(); user_root=(UPLOAD_BOTS_DIR/str(user_id)).resolve()
    if project!=user_root and user_root not in project.parents:
        await callback.answer('❌ Invalid bot path.',show_alert=True); return
    key=f'{user_id}_{entry_file}'
    info=bot_scripts.get(key)
    if info and docker_runner:
        docker_runner.stop_script(info['container_id']); bot_scripts.pop(key,None)
    try:
        project_prefix=project.relative_to(user_root).as_posix().rstrip('/') + '/'
    except ValueError:
        await callback.answer('❌ Invalid bot location.',show_alert=True); return
    shutil.rmtree(project,ignore_errors=True)
    conn=db_connect(); c=conn.cursor(); c.execute('DELETE FROM bot_projects WHERE user_id=? AND bot_name=?',(user_id,bot_name)); c.execute('DELETE FROM user_files WHERE user_id=? AND (file_name=? OR file_name LIKE ?)',(user_id,project_prefix.rstrip('/'),project_prefix+'%')); conn.commit(); conn.close()
    user_files[user_id]=[x for x in user_files.get(user_id,[]) if not (x[0]==project_prefix.rstrip('/') or x[0].startswith(project_prefix))]
    await callback.answer('✅ Bot deleted.',show_alert=True); await callback_check_files(callback)
...
@dp.callback_query(F.data == "admin_commands_list")
async def callback_admin_commands_list(callback: types.CallbackQuery):
    if callback.from_user.id not in admin_ids:
        await callback.answer("❌ Admin only!", show_alert=True)
        return
    text = """
...
/checkchannel — verify channel-gate setup

<b>⚙️ Runtime limits (admin):</b>
/setrunlimit free|paid NUMBER — concurrent runs allowed
/setrunduration free|paid VALUE — max run time (e.g. 30m, 2h)
/setratelimit upload|run PER_HOUR — free-tier throttle

<b>👑 Owner only:</b>
/restart — restart the bot process
/setchannel @user URL — require channel join (or "off")

<i>Admin Panel buttons cover file/user management, logs, cleanup, etc. —
this list is just the slash-commands, since new ones are easy to miss.</i>
"""
    await callback.message.edit_text(
        text,
...
@dp.callback_query(F.data == "admin_channel_gate")
async def callback_admin_channel_gate(callback: types.CallbackQuery):
    if callback.from_user.id not in admin_ids:
        await callback.answer("❌ Admin only!", show_alert=True)
        return
    status = f"🟢 Enabled\nChannel: <code>{REQUIRED_CHANNEL}</code>\nJoin URL: {CHANNEL_JOIN_URL or '(not set)'}" if REQUIRED_CHANNEL else "🔴 Disabled"
    text = f"""
╔═══════════════════════╗
    📢 <b>CHANNEL GATE</b> 📢
╚═══════════════════════╝

When enabled, EVERYONE except the owner must join
the configured channel before using the bot at all.

<b>Status:</b>
{status}

<b>Commands (owner only to change):</b>
<code>/setchannel @username https://t.me/username</code>
<code>/setchannel off</code>
<code>/checkchannel</code> — verify bot is admin there
"""
    await callback.message.edit_text(
        text,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Admin Panel", callback_data="admin_panel")]]),
...
Pick what to clean up. Each shows a count + size before you confirm —
nothing is deleted by opening this menu.
"""
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📅 Files older than 30 days", callback_data="cf_preview:old")],
        [InlineKeyboardButton(text="🚫 Banned users' files", callback_data="cf_preview:banned")],
        [InlineKeyboardButton(text="📄 All .log files", callback_data="cf_preview:logs")],
        [InlineKeyboardButton(text="🔙 Admin Panel", callback_data="admin_panel")]
    ])
    await callback.message.edit_text(text, reply_markup=keyboard, parse_mode="HTML")
    await callback.answer()

def _clean_files_scan(option: str):
    """Returns list of Path objects that WOULD be deleted for this option —
    used for both the preview count and the actual deletion, so what the
    admin sees is exactly what gets acted on."""
    found = []
    if not UPLOAD_BOTS_DIR.exists():
        return found
    if option == 'old':
        cutoff = time.time() - 30 * 24 * 3600
        for p in UPLOAD_BOTS_DIR.rglob("*"):
            if p.is_file():
                try:
        ...
        found.extend(p for p in UPLOAD_BOTS_DIR.rglob("*.log") if p.is_file())
    return found

_CLEAN_FILES_LABELS = {
    'old': "files older than 30 days",
    'banned': "banned users' files",
    'logs': "all .log files",
}

@dp.callback_query(F.data.startswith("cf_preview:"))
async def callback_clean_files_preview(callback: types.CallbackQuery):
    if callback.from_user.id not in admin_ids:
        await callback.answer("❌ Admin only!", show_alert=True)
        return
    option = callback.data.split(":", 1)[1]
    files = _clean_files_scan(option)
    total_bytes = sum((f.stat().st_size for f in files if f.exists()), 0)
    label = _CLEAN_FILES_LABELS.get(option, option)

    if not files:
        await callback.message.edit_text(
            f"🟢 Nothing to clean — no {label} found.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Admin Panel", callback_data="admin_panel")]])
        )
        await callback.answer()
        return
    ...
    text = (
        ...
        f"This will permanently delete <b>{len(files)}</b> file(s) — {label}.\n"
        f"Total size: <b>{total_bytes / (1024*1024):.1f} MB</b>\n\n"
        f"This cannot be undone. Proceed?"
    )
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Yes, delete", callback_data=f"cf_go:{option}"),
         InlineKeyboardButton(text="❌ Cancel", callback_data="admin_clean_files")]
    ])
    await callback.message.edit_text(text, reply_markup=keyboard, parse_mode="HTML")
    await callback.answer()

@dp.callback_query(F.data.startswith("cf_go:"))
async def callback_clean_files_go(callback: types.CallbackQuery):
    if callback.from_user.id not in admin_ids:
        await callback.answer("❌ Admin only!", show_alert=True)
        return
    option = callback.data.split(":", 1)[1]
    ...
    text = f"✅ <b>Cleanup done</b>\n\n🗑 {deleted} file(s) deleted, {freed / (1024*1024):.1f} MB freed."
    if errors:
...
MAX_ADMIN_LOG_LIST = 25  # cap the list so a huge deployment doesn't send an unusable wall of buttons
...
async def callback_admin_view_logs(callback: types.CallbackQuery):
    ...
    if not UPLOAD_BOTS_DIR.exists():
        ...
        log_files = sorted(
            (p for p in UPLOAD_BOTS_DIR.rglob("*.log") if p.is_file()),
    ...
    if not log_files:
        await callback.message.edit_text(
            "📝 No log files found across any user yet.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Admin Panel", callback_data="admin_panel")]])
        )
        await callback.answer()
        return

    shown = log_files[:MAX_ADMIN_LOG_LIST]
    buttons = []
    for p in shown:
        rel = p.relative_to(UPLOAD_BOTS_DIR)
        user_id_part = rel.parts[0] if rel.parts else "?"
        label = f"{user_id_part} · {p.name}"[:60]
        buttons.append([InlineKeyboardButton(text=label, callback_data=f"adminlog:{rel}")])
    buttons.append([InlineKeyboardButton(text="🔙 Admin Panel", callback_data="admin_panel")])

    text = f"📝 <b>All User Logs</b> (most recent first)\n\nShowing {len(shown)} of {len(log_files)} log file(s)."
    await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")
    await callback.answer()

@dp.callback_query(F.data.startswith("adminlog:"))
async def callback_admin_log_view(callback: types.CallbackQuery):
    if callback.from_user.id not in admin_ids:
        await callback.answer("❌ Admin only!", show_alert=True)
        return
    rel = callback.data.split(":", 1)[1]
    path = (UPLOAD_BOTS_DIR / rel).resolve()
    try:
        path.relative_to(UPLOAD_BOTS_DIR.resolve())  # refuse anything that escaped the uploads root
    except ValueError:
        await callback.answer("❌ Invalid log path.", show_alert=True)
        return
    if not path.exists():
        await callback.answer("⚠️ Log file no longer exists.", show_alert=True)
        return

    try:
        content = path.read_text(encoding='utf-8', errors='replace')
    except OSError as e:
        await callback.answer(f"❌ Could not read log: {e}", show_alert=True)
        return

    back_keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Back to Logs", callback_data="admin_view_logs")]])
    if len(content) <= 3500:
        await callback.message.edit_text(
            f"📝 <b>{rel}</b>\n\n<pre>{content[-3500:] or '(empty)'}</pre>",
            reply_markup=back_keyboard, parse_mode="HTML"
        )
        await callback.answer()
    else:
        # too long for a message — send as a document instead
        await callback.answer()
        await callback.message.answer_document(FSInputFile(path), caption=f"📝 {rel}")
...
All running scripts' watchers/state in memory
are lost (the scripts themselves keep running,
process_runner.py reconnects to them on boot).
Users may experience a few seconds of downtime.

<b>Only use if necessary!</b>
"""
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Confirm Restart", callback_data="confirm_restart_bot")],
        [InlineKeyboardButton(text="🔙 Admin Panel", callback_data="admin_panel")]
    ])
    await callback.message.edit_text(text, reply_markup=keyboard, parse_mode="HTML")
    await callback.answer()

@dp.callback_query(F.data == "confirm_restart_bot")
async def callback_confirm_restart_bot(callback: types.CallbackQuery):
    if callback.from_user.id != OWNER_ID:
        await callback.answer("❌ Owner only!", show_alert=True)
        return
    await callback.answer("🔄 Restarting now...", show_alert=True)
    try:
        await callback.message.edit_text("🔄 Bot is restarting — back in a few seconds.")
    except Exception:
        pass
    logger.info(f"Bot restart triggered by owner {callback.from_user.id} via admin panel")
    await asyncio.sleep(1)  # let the message above actually reach Telegram before the process dies
    os._exit(0)  # supervisor's autorestart=true brings the process straight back up
...
async def cmd_restart(message: types.Message):
    if message.from_user.id != OWNER_ID:
        ...
        return
    await message.answer("🔄 Restarting now — back in a few seconds.")
    logger.info(f"Bot restart triggered by owner {message.from_user.id} via /restart")
...
@dp.message(Command("setrunlimit"))
async def cmd_set_run_limit(message: types.Message):
    if message.from_user.id not in admin_ids:
        await message.answer("❌ Permission denied!")
        return
    args = message.text.split()
    if len(args) != 3 or args[1].lower() not in ('free', 'paid'):
        await message.answer(
            f"Usage: /setrunlimit &lt;free|paid&gt; NUMBER\n\n"
            f"Current: free={run_limits['free']}, paid={run_limits['paid']}\n"
            f"(Owner/admins are always unlimited, not affected by this.)",
            parse_mode="HTML"
        )
        return
    tier = args[1].lower()
    try:
        ...
        await message.answer("❌ NUMBER must be a whole number!")
        return
    if value <= 0:
        await message.answer("❌ Limit must be greater than 0!")
        return
    set_run_limit(tier, value)
    await message.answer(
        f"✅ <b>Concurrent run limit updated</b>\n\n"
        f"{'🆓 Free' if tier == 'free' else '💎 Paid'} users can now run up to <b>{value}</b> script(s) at once.\n"
...
async def cmd_set_channel(message: types.Message):
    ...
    if len(args) == 2 and args[1].lower() == 'off':
        set_required_channel(None, None)
        await message.answer("✅ Channel-join requirement disabled — anyone can use the bot now.")
        return

    if len(args) != 3:
        current = f"{REQUIRED_CHANNEL} ({CHANNEL_JOIN_URL})" if REQUIRED_CHANNEL else "disabled"
        await message.answer(
            "Usage:\n"
            "<code>/setchannel @channelusername https://t.me/channelusername</code> — enable\n"
            "<code>/setchannel -1001234567890 https://t.me/+invitehash</code> — enable (private channel)\n"
            "<code>/setchannel off</code> — disable\n\n"
            f"Current: {current}\n\n"
            "⚠️ The bot must be an ADMIN in that channel, or membership checks will fail "
            "(and the gate fails OPEN — it won't lock everyone out, but it also won't work).",
            parse_mode="HTML"
        )
        return

    channel_id, join_url = args[1], args[2]
    warning = ""
    try:
        me = await bot.get_me()
        member = await bot.get_chat_member(channel_id, me.id)
        if member.status not in ('administrator', 'creator'):
            warning = "\n\n⚠️ Bot is in that channel but is NOT an admin there — membership checks will likely fail. Make the bot an admin."
    except Exception as e:
        warning = f"\n\n⚠️ Could not verify the channel right now ({e}). Saved anyway — double check the ID/username and that the bot has been added there as an admin."

    set_required_channel(channel_id, join_url)
    await message.answer(f"✅ Channel-join requirement enabled: <code>{channel_id}</code>{warning}", parse_mode="HTML")

@dp.message(Command("checkchannel"))
async def cmd_check_channel(message: types.Message):
    if message.from_user.id not in admin_ids:
        await message.answer("❌ Permission denied!")
        return
    if REQUIRED_CHANNEL is None:
        await message.answer("Channel-join gate is currently disabled (/setchannel to enable).")
        return
    try:
        me = await bot.get_me()
        bot_member = await bot.get_chat_member(REQUIRED_CHANNEL, me.id)
        your_member = await bot.get_chat_member(REQUIRED_CHANNEL, message.from_user.id)
        await message.answer(
            f"📢 Channel: <code>{REQUIRED_CHANNEL}</code>\n"
            f"🔗 Join URL: {CHANNEL_JOIN_URL or '(not set)'}\n"
            f"🤖 Bot status there: <b>{bot_member.status}</b>\n"
            f"👤 Your status there: <b>{your_member.status}</b>\n\n"
            + ("✅ Setup looks correct." if bot_member.status in ('administrator', 'creator') else "⚠️ Bot is not an admin — checks will fail open (everyone gets in) until fixed."),
            parse_mode="HTML"
        )
    except Exception as e:
        await message.answer(f"❌ Could not check channel: {e}\n\nVerify the channel ID/username and that the bot has been added to it.")

@dp.message(Command("setrunduration"))
async def cmd_set_run_duration(message: types.Message):
    if message.from_user.id not in admin_ids:
        await message.answer("❌ Permission denied!")
        return
    args = message.text.split()
    if len(args) != 3 or args[1].lower() not in ('free', 'paid'):
        ...
        await message.answer(
            f"Usage: /setrunduration &lt;free|paid&gt; VALUE\n\n"
            f"VALUE is seconds by default — add 'm' for minutes or 'h' for hours "
            f"(e.g. 30m, 2h, or a plain number like 1800).\n\n"
            f"Current: free={_fmt(run_durations['free'])}, paid={_fmt(run_durations['paid'])}\n"
            f"(Owner/admins always unlimited — no auto-kill timer at all.)",
            parse_mode="HTML"
        )
        return
    tier = args[1].lower()
    raw = args[2].lower().strip()
    try:
        if raw.endswith('h'):
        ...
        await message.answer("❌ VALUE must be a number, optionally ending in 's', 'm', or 'h' (e.g. 1800, 30m, 2h).")
        return
    if seconds <= 0:
        await message.answer("❌ Duration must be greater than 0!")
        return
    set_run_duration(tier, seconds)
    hours = seconds / 3600
    await message.answer(
        f"✅ <b>Max run duration updated</b>\n\n"
        f"{'🆓 Free' if tier == 'free' else '💎 Paid'} scripts now get auto-killed after "
        f"<b>{hours:.2g} hour(s)</b> ({seconds}s) if still running.\n"
        f"Takes effect immediately for new runs, no restart needed.",
        parse_mode="HTML"
    )

@dp.message(Command("setratelimit"))
async def cmd_set_rate_limit(message: types.Message):
    if message.from_user.id not in admin_ids:
        await message.answer("❌ Permission denied!")
        return
    args = message.text.split()
    if len(args) != 3 or args[1].lower() not in ('upload', 'run'):
        await message.answer(
            f"Usage: /setratelimit &lt;upload|run&gt; PER_HOUR\n\n"
            f"Current: upload={rate_limits['upload']}/hr, run={rate_limits['run']}/hr\n"
            f"(Free tier only — paid/admin/owner are never rate-limited.)",
            parse_mode="HTML"
        )
        return
    kind = args[1].lower()
    try:
        ...
        await message.answer("❌ PER_HOUR must be a whole number!")
        return
    if value <= 0:
        await message.answer("❌ Limit must be greater than 0!")
        return
    set_rate_limit(kind, value)
    await message.answer(
        f"✅ <b>Rate limit updated</b>\n\n"
        f"Free users can now {'upload' if kind == 'upload' else 'run scripts'} up to "
        f"<b>{value}</b> time(s) per hour.\nTakes effect immediately, no restart needed.",
        parse_mode="HTML"
    )
